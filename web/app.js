...
byId("download").addEventListener("click", () => { downloaded = true; byId("backup-note").hidden = true; status("Encrypted identity.pem downloaded", "ok"); setTimeout(revokePemBlobUrl, 0); });
...
function resetGithub() {
  pollAbort?.abort(); byId("result-stage").hidden = true; byId("checking").hidden = true; byId("github-stage").hidden = false; byId("github-form-wrap").hidden = false; byId("repository").value = ""; byId("commit").value = ""; byId("path").value = ""; touchedFields.clear();
...
byId("contribution").addEventListener("submit", async (event) => {
  event.preventDefault(); try {
    ...
    byId("github-form-wrap").hidden = true; byId("checking").hidden = false; byId("checking-repository").textContent = repository; byId("checking-commit").textContent = commit; const started = performance.now(); status("Preparing signed request");
    ...
    const result = await pollReply(reply, { job, did, sha256: await sha256(text) }); if (result.worker !== WORKER_DID || result.request.did !== did) throw new Error("Worker response identity mismatch."); for (const item of byId("execution").children) item.className = "done"; showResult(result, performance.now() - started);
  } catch (error) { byId("checking").hidden = true; byId("github-form-wrap").hidden = false; status(error.name === "AbortError" ? "Polling stopped." : error.message, "error"); }
