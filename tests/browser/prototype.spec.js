...
test("imports PEM, rejects wrong password, validates inputs, optional path, and renders result", async ({ page }) => {
  ...
  await page.locator("#check-another").click(); await expect(page.locator("#github-form-wrap")).toBeVisible(); await expect(page.locator("#did")).toHaveText(did); await expect(page.locator("#identity-stage")).toBeVisible(); await expect(page.locator("#identity-setup")).toBeHidden();
});
...
test("renders a shareable receipt and exposes source metadata", async ({ page }) => {
  ...
  await page.locator("#example").click(); await page.locator("#send").click(); await expect(page.locator("#shareable-receipt")).toBeVisible(); await expect(page.locator("#result-repository")).toHaveText("CONFIRMED"); await expect(page.locator("#result-file")).toBeHidden(); await expect(page.locator("body")).not.toContainText("VERIFIED");
  const digest = await page.locator("#provenance-digest").textContent(); const expectedId = `${digest.slice(0, 4).toUpperCase()}-${digest.slice(4, 8).toUpperCase()}`; await expect(page.locator("#receipt-id")).toHaveText(expectedId); await expect(page.locator("#identity-stage")).toBeHidden(); await expect(page.locator("#control-stage")).toBeHidden();
  ...
  const firstDownload = page.waitForEvent("download"); await page.locator("#download-receipt").click(); const png = await (await firstDownload).createReadStream().then(async (stream) => { const parts = []; for await (const part of stream) parts.push(part); return Buffer.concat(parts); }); expect(png.readUInt32BE(16)).toBe(1200); expect(png.readUInt32BE(20)).toBe(675); expect(await page.evaluate(() => window.__canvasText.some((value) => value.startsWith("Requester  did:key:")))).toBe(true); expect(await page.evaluate(() => window.__canvasText.includes("CHECK RECEIPT"))).toBe(false); expect(await page.evaluate(() => window.__canvasText.includes("FILE"))).toBe(false); expect(await page.evaluate(() => window.__canvasText.includes("NOT REQUESTED"))).toBe(false);
  await page.evaluate(() => { window.__canvasText = []; }); await page.getByText("Include my DID", { exact: true }).click(); await expect(page.locator("#include-did-image")).not.toBeChecked(); const secondDownload = page.waitForEvent("download"); await page.locator("#download-receipt").click(); await secondDownload; expect(await page.evaluate(() => window.__canvasText.includes("Requester DID hidden"))).toBe(true); expect(await page.evaluate(() => window.__canvasText.some((value) => value.startsWith("Requester  did:key:")))).toBe(false);
  const ids = await page.evaluate(async () => { const module = await import("/receipt.js"); return [module.receiptId("a".repeat(64)), module.receiptId("b".repeat(64))]; }); expect(ids).toEqual(["AAAA-AAAA", "BBBB-BBBB"]);
  const longRepositoryLines = await page.evaluate(async () => { window.__canvasText = []; const module = await import("/receipt.js"); const canvas = document.createElement("canvas"); await module.drawReceipt(canvas, { repository: "very-long-owner-name/an-intentionally-long-github-repository-name-for-responsive-social-card-rendering", commit: "93dab08e185121186d009f9b637a37365c294ea1", repositoryStatus: "CONFIRMED", commitStatus: "CONFIRMED", fileStatus: "NOT REQUESTED", requesterDid: "did:key:z6Mkrequester", workerDid: "did:key:z6Mkworker", receiptId: "AAAA-AAAA", duration: "3.6s" }); return { size: [canvas.width, canvas.height], lines: window.__canvasText }; }); expect(longRepositoryLines.size).toEqual([1200, 675]); expect(longRepositoryLines.lines.filter((value) => value.includes("long") || value.includes("responsive"))).toHaveLength(3);
  const fileLine = await page.evaluate(async () => {
    const module = await import("/receipt.js");
    const base = { repository: "owner/repo", commit: "c".repeat(40), repositoryStatus: "CONFIRMED", commitStatus: "CONFIRMED", fileStatus: "NOT REQUESTED", requesterDid: "did:key:z6Mkr", workerDid: "did:key:z6Mkw", receiptId: "AAAA-AAAA", duration: "1.0s" };
    window.__canvasText = []; await module.drawReceipt(document.createElement("canvas"), { ...base }); const imageNoPath = window.__canvasText.slice();
    window.__canvasText = []; await module.drawReceipt(document.createElement("canvas"), { ...base, path: "docs/x.md", fileStatus: "CONFIRMED" }); const imageWithPath = window.__canvasText.slice();
    return { textNoPath: module.copyReceiptText({ ...base }), textWithPath: module.copyReceiptText({ ...base, path: "docs/x.md", fileStatus: "CONFIRMED" }), imageNoPath, imageWithPath };
  });
  expect(fileLine.textNoPath).not.toContain("File:"); expect(fileLine.textWithPath).toContain("File: CONFIRMED");
  expect(fileLine.imageNoPath).not.toContain("FILE"); expect(fileLine.imageWithPath).toContain("FILE");
  expect(consoleErrors).toEqual([]); expect(requests.every((url) => new URL(url).origin === "http://127.0.0.1:18787")).toBe(true); expect(copied).not.toContain(PASSPHRASE); expect(copied).not.toContain("PRIVATE KEY"); expect((await page.evaluate(() => window.__canvasText.join("\n")))).not.toContain(PASSPHRASE);
...
test("continues to the GitHub check without downloading identity.pem and shows no dialog", async ({ page }) => {
  const did = await createIdentity(page);
  let dialogMessage = null;
  page.on("dialog", (dialog) => { dialogMessage = dialog.message(); return dialog.dismiss().catch(() => {}); });
  // identity.pem was never downloaded; the reminder is inline and non-blocking
  await expect(page.locator("#download")).toBeVisible();
  await expect(page.locator("#backup-note")).toBeVisible();
  await mockV2Reply(page, did, { repository: { status: "CONFIRMED" }, commit: { status: "CONFIRMED" }, requested_file: { status: "NOT_CHECKED" } }, { baseline: 0, sequence: 2 });
  await page.locator("#example").click();
  await page.locator("#send").click();
  // proceeds straight through with no window.confirm()
  await expect(page.locator("#github-form-wrap")).toBeHidden();
...
test("re-hides a revealed wallet recovery phrase when the workflow advances", async ({ page }) => {
  ...
  await page.locator("#example").click(); await page.locator("#send").click();
...
test("mobile layout fits and JavaScript-disabled message is useful", async ({ browser }) => {
  const mobileContext = await browser.newContext({ viewport: { width: 390, height: 844 } }); const mobile = await mobileContext.newPage(); const did = await createIdentity(mobile); await mockV2Reply(mobile, did, { repository: { status: "CONFIRMED" }, commit: { status: "CONFIRMED" }, requested_file: { status: "NOT_CHECKED" } }, { baseline: 0, sequence: 1 }); await mobile.locator("#example").click(); await mobile.locator("#send").click(); await expect(mobile.locator("#shareable-receipt")).toBeVisible(); expect(await mobile.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true); await expect(mobile.locator("#receipt-repository")).toBeVisible(); await expect(mobile.locator("#copy-receipt")).toBeVisible(); await mobileContext.close();
