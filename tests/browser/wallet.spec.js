...
test("wallet secrets remain isolated from Technocore and receipt surfaces", async ({ page, browserName }) => {
  ...
  await page.locator("#new-passphrase").fill(PASSPHRASE); await page.locator("#confirm-passphrase").fill(PASSPHRASE); await page.locator("#create").click(); await expect(page.locator("#proof-result")).toHaveText("DID control demonstrated"); const did = await page.locator("#did").textContent(); let expected;
