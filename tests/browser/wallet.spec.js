...
test("Copy recovery phrase copies the mnemonic and reuses it as the identity passphrase", async ({ page, context }) => {
  await page.addInitScript(() => {
    Math.random = () => { throw new Error("Math.random must not be used"); };
    window.__copied = [];
    Object.defineProperty(navigator, "clipboard", { value: { writeText: async (v) => { window.__copied.push(v); } }, configurable: true });
  });
  const requests = [];
  ...
  // wallet is created normally
  await page.locator("#wallet-create").click();
  await expect(page.locator("#wallet-ready")).toBeVisible();
  const address = await page.locator("#wallet-address").textContent();
  expect(address).toMatch(/^0x[0-9a-fA-F]{40}$/);

  // phrase (and the copy action) stay hidden until the explicit reveal + acknowledgement flow
  await expect(page.locator("#wallet-phrase")).toBeHidden();
  await expect(page.locator("#wallet-phrase-list li")).toHaveCount(0);
  await expect(page.locator("#wallet-reveal-confirm")).toBeHidden();
  await expect(page.locator("#wallet-copy-phrase")).toBeHidden();
  await expect(page.locator("#new-passphrase")).toHaveValue("");
  expect(await page.evaluate(() => window.__copied.length)).toBe(0);

  await reveal(page);
  const phrase = await mnemonic(page);
  expect(phrase.split(" ")).toHaveLength(12);

  const before = requests.length;
  await page.locator("#wallet-copy-phrase").click();

  // one click -> clipboard and Passphrase field both get the exact same mnemonic
  expect(await page.evaluate(() => window.__copied.at(-1))).toBe(phrase);
  await expect(page.locator("#new-passphrase")).toHaveValue(phrase);
  await expect(page.locator("#new-passphrase")).toHaveAttribute("type", "password");
  await expect(page.locator("#wallet-feedback")).toHaveText("Copied & added to Passphrase ✓");

  // no network, no persistence from the action
  expect(requests.length).toBe(before);
  expect(requests.length).toBe(loaded);
  expect(await page.evaluate(() => ({ l: localStorage.length, s: sessionStorage.length, url: location.href }))).toEqual({ l: 0, s: 0, url: "http://127.0.0.1:18787/" });
  const cookies = await context.cookies();
  expect(JSON.stringify(cookies)).not.toContain(phrase);

  // the mnemonic itself is unchanged (not regenerated) and still matches the wallet
  expect(await mnemonic(page)).toBe(phrase);
  await expect(page.locator("#wallet-address")).toHaveText(address);
});

test("Copy recovery phrase still fills the Passphrase when the clipboard fails, without regenerating", async ({ page }) => {
  await page.addInitScript(() => {
  ...
  await page.locator("#wallet-copy-phrase").click();
  await expect(page.locator("#new-passphrase")).toHaveValue(phrase);      // still filled with the existing phrase
  await expect(page.locator("#wallet-feedback")).toHaveText("Added to Passphrase — copy failed");
  await expect(page.locator("#wallet-address")).toHaveText(address);      // wallet not regenerated
  expect(await mnemonic(page)).toBe(phrase);                             // mnemonic not regenerated

  await page.locator("#wallet-copy-phrase").click();                     // repeat click does not regenerate
  await expect(page.locator("#new-passphrase")).toHaveValue(phrase);
  await expect(page.locator("#wallet-address")).toHaveText(address);
  expect(await mnemonic(page)).toBe(phrase);
});
...
test("wallet secrets remain isolated from Technocore and receipt surfaces", async ({ page, browserName }) => {
  ...
  // deliberately link the recovery phrase into the identity passphrase, then prove it still does not leak
  await page.locator("#wallet-copy-phrase").click(); await expect(page.locator("#new-passphrase")).toHaveValue(secret);
  await page.locator("#wallet-hide").click();
  await page.locator("#create").click(); await expect(page.locator("#proof-result")).toHaveText("DID control demonstrated"); const did = await page.locator("#did").textContent(); let expected;
