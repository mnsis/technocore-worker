...
const enabled = process.env.TC_E2E === "1"; const base = process.env.TC_E2E_URL; const pemPath = "/tmp/technocore-phase4-e2e-identity.pem";
test("one controlled production contribution check", async ({ page, browserName }) => {
  ...
  await page.goto(base); await page.locator("#new-passphrase").fill(passphrase); await page.locator("#create").click(); await expect(page.locator("#proof-result")).toHaveText("DID control demonstrated"); const did = await page.locator("#did").textContent();
  ...
  await import("node:fs").then(({ writeFileSync }) => writeFileSync("/tmp/technocore-phase4-e2e-result.json", JSON.stringify({ did, latency, requests: requests.map(({ url }) => url), worker }), { mode: 0o600 }));
});
