...
test("one controlled production contribution check", async ({ page, browserName }) => {
  ...
  await page.goto(base); await page.locator("#new-passphrase").fill(passphrase); await page.locator("#confirm-passphrase").fill(passphrase); await page.locator("#create").click(); await expect(page.locator("#status")).toHaveText("Identity ready"); const did = await page.locator("#did").textContent();
  ...
  await page.locator("#prove").click(); await expect(page.locator("#proof-result")).toHaveText("✓ DID control demonstrated"); await page.locator("#example").click(); await page.locator("#send").click(); await expect(page.locator("#status")).toHaveText("Response received", { timeout: 160000 });
